...
namespace aliyunTest.Framework
{
    ...
    public static class StringExtensions
    {
        ...
        public const String EncodingString = "eilntinc";
        public const String CookiesKey = "1643C83CFDA68001";
        private static Regex YesInt = new Regex("^[1-9][0-9]*$"); //正整数判断
        private static Regex NoInt = new Regex("^[0-9]*$"); //整数判断
        private static Regex YesPrice = new Regex(@"(^\d*\.?\d*[0-9]+\d*$)|(^[0-9]+\d*\.\d*$)"); //价格判断 
        private static Regex YesSpacia = new Regex(@"(\r\n)", RegexOptions.IgnoreCase);
        private static aliyunTest.Framework.Common.IPAddress _IPSearch = __getIPConfig();

        private static aliyunTest.Framework.Common.IPAddress __getIPConfig()
        {
            try
            {
                if (HttpContext.Current != null)
                    return new Common.IPAddress(HttpContext.Current.Server.MapPath(Config.FrameworkConfig.Instance<Config.FrameworkConfig>().IPDatabasePath));
                else
                    return new Common.IPAddress(System.IO.Path.Combine((AppDomain.CurrentDomain.RelativeSearchPath ?? AppDomain.CurrentDomain.BaseDirectory), Config.FrameworkConfig.Instance<Config.FrameworkConfig>().IPDatabasePath));
            }
            catch { return null; }
        }
        ...
        public static bool IsRightString(this string param)
        {
            ...
            {
                str_Regex += string.Format(" {0} ", strBadChar[i]) + "|";
            }
            ...
            //里面定义恶意字符集合
            //验证inputData是否包含恶意集合
            if (Regex.IsMatch(param.ToUpper(), str_Regex.ToUpper()))
            {
                return true;
            }
            else
        ...
        public static string GetUserIP()
        {
            ...
            {
                userIP = HttpContext.Current.Request.ServerVariables["HTTP_X_FORWARDED_FOR"].Split(',')[0];
            }
            return userIP;
        }
        ...
        public static string GetFileExtension(this string obj)
        {
            ...
            try
            {
                ...
                {
                    string[] temp = obj.Split(new string[] { "." }, StringSplitOptions.RemoveEmptyEntries);
        ...
        {
            return new Regex(@"<[^>]+>|</[^>]+>").Replace(str, "").Replace("&nbsp;", "").Replace("&copy;", "").ClearBR().ToString();
        }
        ...
        /// 记录程序调试信息,默认为一般错误
        /// </summary> 
        public static void TxtLogger(this string title, Exception ex)
        {
            //未实现
        }
        ...
        public static string IPAddress(this string obj)
        {
            try
            {
                return _IPSearch.IPLocation(obj);
            }
            catch (Exception ex)
            {
                "IP地址对应区域".TxtLogger(ex);
            }
        ...
        /// <param name="obj"></param>
        /// <returns></returns>
        public static string CreateSN(this string obj)
        {
            return obj.Trim().ToUpper() + DateTime.Now.ToString("yyMMddHHmmssfffffff");
        }
        ...
        /// </summary>
        /// <param name="obj"></param>
        /// <param name="attchStr">附加字符</param>
        /// <returns></returns>
        public static string CreateSN(this string obj, object attchStr)
        {
            return obj.Trim().ToUpper() + DateTime.Now.ToString("yyMMddHHmmssfffffff") + attchStr;
        }
